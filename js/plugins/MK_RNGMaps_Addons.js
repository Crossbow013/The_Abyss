/*:
 * @target MZ
 * @plugindesc [Tier 5] [1.23] RNGMaps Addons
 * @author Aerosys
...
DataManager.makeSaveContents = function() {
    ...
    contents.MK.injectedDataMap         = MK.injectedDataMap;
...
DataManager.extractSaveContents = function(contents) {
    ...
    if (contents.MK) {
        ...
        MK.injectedMapId        = contents.MK.injectedDataMap ? $gameMap.mapId() : null;
        MK.injectedDataMap      = contents.MK.injectedDataMap;
    }
}
...
const alias5 = DungeonGenerator.prototype.spawnPlayerAt;
DungeonGenerator.prototype.spawnPlayerAt = function(x, y) {
    
    if (typeof x == 'number' && typeof y == 'number') {
        this.spawnLocation = { x: x, y: y};
        return this;
    }
    return alias5.call(this, x, y);
}
...
if (PluginManager && PluginManager.registerCommand) {
    ...
    PluginManager.registerCommand('MK_RNGMaps_Addons', 'spawnPlayerAt', args => {
        $dungeonGenerator.spawnPlayerAt(Number(args.x), Number(args.y));
        
        if (MK.$builder)
            MK.$builder.spawnPlayerAt(Number(args.x), Number(args.y));
    });

    PluginManager.registerCommand('MK_RNGMaps_Addons', 'spawnPlayerAtVariables', args => {
        const x = $gameVariables.value(Number(args.x));
        const y = $gameVariables.value(Number(args.y));
        $dungeonGenerator.spawnPlayerAt(x, y);

        if (MK.$builder)
            MK.$builder.spawnPlayerAt(x, y);
    });
