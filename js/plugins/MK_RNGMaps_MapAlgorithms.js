/*:
 * @target MZ
 * @plugindesc [Tier 4] [0.3 ] (no Plugin Parameters or Commands)
 * @author Aerosys
...
if (!Imported.MK_RNGMaps_withoutSnippets)
    alert ("Missing Plugin: MK_RNGMaps_withoutSnippets");
...
mapGenerationAlgorithms.staticMap = function (dataMap, args) {
    ...
    if (args.spawnLocationArguments.length == 2
...
mapGenerationAlgorithms.worldMap = function (dataMap, args) {
    ...
    args.n_islands          = args.n_islands || [0, 99];
    if (!Array.isArray(args.n_islands))
        args.n_islands = [args.n_islands, args.n_islands];
    ...
    args.riverTileStack = args.riverTileStack || [ MK.tileFromEditorsTable('A1', 0, 0), 0, 0, 0, 0, 0 ];
    args.townsVariable  = args.townsVariable || 400;
    const spawnLocationOptions = [];

    // --- code starts here ---

    function inFrame (x, y) { return 0 <= x && x < dataMap.width && 0 <= y && y < dataMap.height }
...
mapGenerationAlgorithms.cave = function(dataMap, args) {
    ...
    args.n_nodes                = args.n_nodes || args.n_nodes === 0
                                    ? args.n_nodes
                                    : [5, 15];
    if (!Array.isArray(args.n_nodes))
        args.n_nodes            = [args.n_nodes, args.n_nodes];
    args.n_randomEdges          = args.n_randomEdges || args.n_randomEdges === 0
                                    ? args.n_randomEdges
                                    : [0, 2];
    if (!Array.isArray(args.n_randomEdges))
        args.n_randomEdges      = [args.n_randomEdges, args.n_randomEdges];
    args.thickness              = args.thickness || [3, 5];
    if (!Array.isArray(args.thickness))
        args.thickness          = [args.thickness, args.thickness];
    args.roadThickness          = args.roadThickness || [4, 5];
    if (!Array.isArray(args.roadThickness))
        args.roadThickness      = [args.roadThickness, args.roadThickness];
    ...
    if (!this.spawnLocation)
        throw Error("Cave Generation does not support spawnLocation: " + args.spawnLocationArguments);
...
mapGenerationAlgorithms.roomsAndCorridors = function(dataMap, args) {
    ...
    args.n_roomsToBuild     = args.n_roomsToBuild || [4, 5];
    args.roomWidth          = args.roomWidth || [7, 9];
    args.roomHeight         = args.roomHeight || [5, 5];
    if (!Array.isArray(args.n_roomsToBuild))
        args.n_roomsToBuild = [args.n_roomsToBuild, args.n_roomsToBuild];
    if (!Array.isArray(args.roomWidth))
        args.roomWidth      = [args.roomWidth, args.roomWidth];
    if (!Array.isArray(args.roomHeight))
        args.roomHeight     = [args.roomHeight, args.roomHeight];
    args.allowOverlapping   = !!args.allowOverlapping;
    args.n_randomEdges      = args.n_randomEdges || args.n_randomEdges === 0
                                ? args.n_randomEdges
                                : [0, 2];
    if (!Array.isArray(args.n_randomEdges))
        args.n_randomEdges  = [args.n_randomEdges, args.n_randomEdges];
    args.thickness          = args.thickness || [2, 3];
    if (!Array.isArray(args.thickness))
        args.thickness      = [args.thickness, args.thickness];
    ...
    function isOverlapping (rect1, rect2) {
        let b1 = (rect1.x > rect2.x + rect2.width  + 1) || (rect2.x > rect1.x + rect1.width  + 1);
        let b2 = (rect1.y > rect2.y + rect2.height + 1) || (rect2.y > rect1.y + rect1.height + 1);
        return !(b1 || b2);
    }
